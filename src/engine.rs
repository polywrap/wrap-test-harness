use crate::error::{BuildError, ExecutionError, TestError};
...
impl Engine {
    ...
    pub async fn execute(
        ...
        let project_generator_executor: Executor =
            Box::new(move |feature, implementation, subpath| {
                ...
                Box::pin(async move {
                    generator
                        ...
                        .await
                        .map_err(ExecutionError::GenerateError)
                })
            });
        ...
        let build_executor: Executor = Box::new(move |feature, implementation, subpath| {
            let e = engine.clone();
            Box::pin(async move {
                e.build(
                    feature.as_str(),
                ...
                .await
                .map_err(ExecutionError::BuildError)
            })
        });
        ...
        let engine = Arc::new(self.clone());
        let test_executor: Executor = Box::new(move |feature, implementation, subpath| {
            let e = engine.clone();
            Box::pin(async move {
                if let Some(i) = implementation {
                    e.test(feature.as_str(), &i, subpath.as_deref())
                        .await
                        .map_err(ExecutionError::TestError)
                } else {
                    Ok(())
                }
            })
    ...
    async fn handler(
        ...
        match feature {
            None => {
                feature_map =
                    fs::read_dir(&self.path.source)?.fold(feature_map, |mut current, f| {
                        current.insert(
            ...
            match implementation.clone() {
                None => {
                    if implementation_folder.exists() {
                        let implementations = fs::read_dir(implementation_folder)?
                            .map(|i| i.unwrap().file_name().into_string().unwrap())
                            .collect::<Vec<String>>();
                        feature_map.insert(feature.to_string(), CaseType::Simple(implementations));
                    } else {
                        let mut complex_case_map: ComplexCase = HashMap::new();
                        fs::read_dir(feature_folder)?
                            .into_iter()
                            .filter(|i| i.as_ref().unwrap().metadata().unwrap().is_dir())
                            .for_each(|entry| {
                                ...
                                if step_implementations.exists() {
                                    let implementations = fs::read_dir(step_implementations)
                                        .unwrap()
                                        .map(|i| i.unwrap().file_name().into_string().unwrap())
                                        .collect::<Vec<String>>();
                        ...
                        feature_map
                            .insert(feature.to_string(), CaseType::Complex(complex_case_map));
                    };
                }
                Some(i) => {
                    if implementation_folder.exists() {
                        feature_map
                            .insert(feature.to_string(), CaseType::Simple(vec![i.to_string()]));
                    } else {
                        let mut complex_case_map: ComplexCase = HashMap::new();
                        fs::read_dir(feature_folder)?
                            .into_iter()
            ...
            match f.unwrap() {
                CaseType::Simple(implementations) => {
                    for implementation in implementations {
                        current_executions.push(executor(
                            feature.clone(),
        ...
        let execution_futures = features_execution_futures
            .into_values()
            .map(|f| tokio::spawn(join_all(f)));
        let futures = try_join_all(
            futures::stream::iter(execution_futures)
                .collect::<Vec<_>>()
                .await,
        )
        .await
    ...
    async fn build(
        ...
        if let Some(i) = implementation {
            ...
            directory = directory.join("implementations").join(i);
            debug!("Building wrapper in path: {}", directory.to_str().unwrap());
        } else {
            debug!(
                "Building interface in path: {}",
        ...
        if let Ok(output) = build.output() {
            let message = if let Some(i) = implementation {
                if let Some(s) = subpath {
                    format!(
                        "Build of feature: {}, with implementation: {} and subpath: {}",
                        feature, i, s
                    )
                } else {
            ...
            if output.status.code() == Some(0) {
                ...
                return Err(BuildError::BuildExecutionError(format!(
                    "{} failed with error: {}",
                    message,
            ...
            return Err(BuildError::BuildExecutionError(format!(
                "Error on polywrap cli build command: {:?}",
                error
            )));
        }
    ...
    async fn test(
        ...
        let cli = PolywrapCli::new();
        let mut command = cli.command.unwrap();
        let test = command.arg("test").arg("-o").arg("./output.json");
        ...
        if let Some(p) = subpath {
            ...
            directory = directory.join("implementations").join(implementation);
            test.arg("-m").arg("../../polywrap.test.yaml");
            let custom_config = directory.join("../../client-config.ts").exists();
            if custom_config {
                test.arg("-c").arg("../../client-config.ts");
        ...
        if let Ok(f) = fs::read(&info_path) {
            ...
            results
                ...
                .or_default()
                .insert(feature.to_string(), summary);
            let results_file = fs::OpenOptions::new().write(true).open(&info_path).unwrap();
            serde_json::to_writer_pretty(results_file, &results).unwrap();
        } else {
            let mut results = Results::new();
            let summaries = HashMap::from([(feature.to_string(), summary)]);
